...
create index d_obj_ind_cls
    on drtu.d_obj (cls);

create index d_obj_ind_kind
    on drtu.d_obj (kind);

create table drtu.d_rail
(
    id   char not null
...
create index dev_i_d_nkip
    on drtu.dev (d_nkip);

create index dev_i_obj_code
    on drtu.dev (obj_code);

create index dev_ind_d_tkip
    on drtu.dev (d_tkip);

create index dev_ind_devid
    on drtu.dev (devid);

create unique index dev_ind_id_obj
    on drtu.dev (id_obj);

create index dev_ind_num
    on drtu.dev (num);

create index dev_ind_oksend
    on drtu.dev (ok_send);

create table drtu.dev_obj
(
...
create index dev_obj_i_obj_code
    on drtu.dev_obj (obj_code);

create index dev_obj_ind_nplace
    on drtu.dev_obj (region, locate, nplace);

create index dev_obj_ind_oksend
    on drtu.dev_obj (ok_send);



create table drtu.dev_test
(
    cnt      numeric(10),
    id       numeric(10),
    devid    numeric(10),
...
create index ind_dev_zam
    on drtu.dev_zam (obj_code);

create table drtu.pers
(
...
create index ind_pribory_id_rtu
    on drtu.pribory (id_rtu);

create index ind_pribory_obj_code
    on drtu.pribory (obj_code);

create table drtu.s_calendar
(
    myear varchar(4)
);

create table drtu.s_dev
(
    id       numeric(10) not null
...
create index s_dev_ind_scode
    on drtu.s_dev (scode);

create index s_dev_ind_type
    on drtu.s_dev (dtype);

create table drtu.s_dev_new
(
    id       numeric(10) not null
...
create sequence drtu.seq_devid
    start with 300000
    maxvalue 999999;
...
insert into drtu.s_dev (id, grid, dtype, mtest, rtime, ttime, narg, ngold, nplat, nalk, name, d_create, plant, scode,
                        tag1, tag2)
values (10708310, 7, 'СТ-4', 600, 0.780, 0.260, 0.000, 0.000, 0.000, 0.000, null, '2007-04-18', null, '+', null, null);
insert into drtu.s_dev (id, grid, dtype, mtest, rtime, ttime, narg, ngold, nplat, nalk, name, d_create, plant, scode,
                        tag1, tag2)
values (10102201, 1, 'НМШ2-900', 180, 1.650, 0.200, 2.222, 0.000, 0.000, 0.000, null, '2007-04-18', null, '+', null,
        null);
